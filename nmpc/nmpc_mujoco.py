...
def main(control_pub):
    ...
    pref = np.zeros((3, t.shape[0]), dtype = np.double)
    pref_dot = np.zeros((3, t.shape[0]), dtype = np.double)
    # Linear Velocities
    pref[0, :] = 1*(np.cos(1.5*t))
    pref[1, :] = 1*(np.sin(1.5*t))
    pref[2, :] = 0.2

    pref_dot[0,:] = 1 * 1.5 * -(np.sin(1.5*t))
    ...
    plot_states_reference(fig16, ax16, ax26, ax36, h[0:3,:], pref[0:3,:], t, "Reference_trajectory")
