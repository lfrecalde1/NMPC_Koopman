...
import scipy.io
from nmpc import euler_p_f, rbf, lift_Fun_angular, lift_Fun_linear, lift_Fun, f_angular_system, create_ocp_solver_description, lift_Fun_simplified
import rospy
...
def get_system_velocity_sensor():
    ...
    rot = R.from_quat(q)
    rot = rot.as_matrix()
    v_world = np.array([[vx_real], [vy_real], [vz_real]], dtype=np.double)
    rot_inv = np.linalg.inv(rot)
    v_body = rot_inv@v_world
    x = np.array([v_body[0,0], v_body[1, 0], vz_real, wx_real, wy_real, wz_real], dtype=np.double)
    return x
...
def main(control_pub):
    ...
    # Desired Values
    xref = np.zeros((23, t.shape[0]), dtype = np.double)

    xref[0, :] = 3.5*np.cos(t)*np.sin(0.5*t)+0.3*np.cos(0.7*t)*np.cos(0.3*t);
    xref[1, :] = 3.5*np.sin(t)*np.sin(0.5*t)+0.3*np.cos(0.7*t)*np.cos(0.3*t);
    xref[2, :] = 0.3*np.cos(t)*np.sin(0.5*t)+0.0*np.cos(0.7*t)*np.cos(0.3*t);
    xref[5, :] = 0.3*np.sin(t)*np.sin(0.5*t)+0.3*np.cos(0.7*t)*np.cos(0.3*t);

    #xref[3,:] = 0.0
    #xref[4,:] = 0.0
    #xref[5,:] = 0.5*np.sin(0.5*t)
    ...
    x_lift = lift_Fun_simplified(x[:, 0], cent_l, cent_lz)
    ...
    ocp = create_ocp_solver_description(x_lift, N_prediction, t_prediction, vx_max, vx_min, vy_max, vy_min, vz_max, vz_min, psi_p_max, psi_p_min)
    ...
    for k in range(0, t.shape[0]- N_prediction):
        ...
        tic = time.time()
        x_lift = lift_Fun_simplified(x[:, k], cent_l, cent_lz)
        acados_ocp_solver.set(0, "lbx", x_lift)
        acados_ocp_solver.set(0, "ubx", x_lift)

        # Update yref
        for j in range(N_prediction):
            #xref[0:3, k+j] = np.tanh(pref[0:3, k] - h[0:3, k]) + pref_dot[0:3, k]
            yref = xref[:,k+j]
            acados_ocp_solver.set(j, "yref", yref)
        #xref[0:3, k+N_prediction] = np.tanh(pref[0:3, k] - h[0:3, k]) + pref_dot[0:3, k]
        yref_N = xref[:,k+N_prediction]
        acados_ocp_solver.set(N_prediction, "yref", yref_N[0:19])
        # Get Computational Time
        ...
        x[3:6, k+1] = hp[3:6, k+1]
        x[6:9, k+1] = h[7:10, k+1]
    ...
    plot_states_reference_angular(fig14, ax14, ax24, ax34, x[3:6,:], xref[3:6,:], t, "Reference_angular")
