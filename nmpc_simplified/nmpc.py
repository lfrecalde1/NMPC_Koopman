...
def lift_Fun_simplified(x, cent_l, cent_lz):
    x_lift = []
    for k in x: x_lift.append(k)
    for k in rbf(x[0], cent_l, "gauss"): x_lift.append(k)
    for k in rbf(x[1], cent_l, "gauss"): x_lift.append(k)
    for k in rbf(x[2], cent_lz, "gauss"): x_lift.append(k)
    for k in rbf(x[4], cent_l, "gauss"): x_lift.append(k)
    for k in rbf(x[5], cent_lz, "gauss"): x_lift.append(k)
    x_lift = np.array(x_lift)
    return x_lift
...
def create_ocp_solver_description(x0, N_horizon, t_horizon, vx_max, vx_min, vy_max, vy_min, vz_max, vz_min, psi_p_max, psi_p_min) -> AcadosOcp:
    ...
    R_mat = 5 * np.diag([(1/vx_max), (1/vy_max), (1/vz_max), (1/psi_p_max)])
    ...
    # set constraints
    ocp.constraints.lbu = np.array([vx_min, vy_min, vz_min, psi_p_min])
    ocp.constraints.ubu = np.array([vx_max, vy_max, vz_max, psi_p_max])
